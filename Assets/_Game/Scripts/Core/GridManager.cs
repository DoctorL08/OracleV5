...
public class GridManager : MonoBehaviour
{
    ...
    // Isometric diamond picking — only WALKABLE cells are candidates.
    // Non-walkable cells (obstacles, walls) are skipped entirely: clicking anywhere
    // on or near an obstacle cell returns null, so no movement or spell can target it.
    // Adjacent walkable cells remain fully selectable via their own diamond zones.
    public Cell GetCellAtScreenPosition(Camera cam, Vector2 screenPos)
    {
        ...
        worldPos.x -= config.mousePickOffset.x;
        worldPos.y -= config.mousePickOffset.y;
        ...
        for (int dy = -2; dy <= 2; dy++)
        {
            ...
            if (cell == null || !cell.IsWalkable) continue;  // obstacle cells ignored
